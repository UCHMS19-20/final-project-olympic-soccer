...
while True:
    ...
    if speed[1] == 0:    
        if ball_rect.left <= zeus_rect_pos[0] <= ball_rect.right or ball_rect.left <= poseidon_rect_pos[0] <= ball_rect.right:
            ...
            if ball_rect.bottom >= zeus_rect.top or ball_rect.bottom >= poseidon_rect.top:
                speed[1] = 40
    if speed[0] == 0:
        if ball_rect.top <= zeus_rect_pos[1] <= ball_rect.bottom or ball_rect.top <= poseidon_rect_pos[1] <= ball_rect.bottom:   
    ...
    if ball_rect.top <= zeus_rect_pos[1] <= ball_rect.bottom:
        if ball_rect.left <= zeus_rect_pos[0] <= ball_rect.right:
            speed[0] -= speed[0]
    ...
    if ball_rect.top <= poseidon_rect_pos[1] <= ball_rect.bottom:
        if ball_rect.left <= poseidon_rect_pos[0] <= ball_rect.right:
            speed[0] -= speed[0]
    ...
    if ball_rect.left <= soccer_field_rect.left or ball_rect.right >= soccer_field_rect.right:
        speed[0] -= speed[0]
    if ball_rect.top <= soccer_field_rect.top or ball_rect.bottom >= soccer_field_rect.bottom:
        speed[1] -= speed[1]
